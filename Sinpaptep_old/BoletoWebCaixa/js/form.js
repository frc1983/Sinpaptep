...
String.prototype.trimLeadingZeros = function (todos) { //true, false
    ...
    var i = 0;
    while ((i < this.length - (todos?0:1)) && (this.substring(i, i + 1) == '0'))
        i++;
...
function formataDataDigitada(campo) {
    ...
    for (var tam = 0; tam < valor.length; tam++) {
        if (j == 0) {
            ...
            if ((tam == 1) && (valor.length > 2)) {
                j++;
                temp += "/";
            }
        } else if (j == 1) {
            temp += valor.substring(tam, tam + 1);
            if ((tam == 3) && (valor.length > 4)) {
                j++;
                temp += "/";
            }
        } else if (j == 2) {
...
function Consiste(theForm)
{
    ...
    {
        alert("Informar o campo Nome da Empresa com 3 a 40 caracteres");
        theForm.sacadoNome.focus();
        return (false);
    }

    if ((!Valido(theForm.sacadoCNPJ.value, "0123456789-/.") || theForm.sacadoCNPJ.value == ""))//|| (theForm.sacadoCep.value.length < 9))
    {
        alert("Informar o campo CNPJ no formato 00.000.000/0000-00");
        theForm.sacadoCNPJ.focus();
        return (false);
    }
    ...
    if ((!Valido(theForm.sacadoCep.value, "0123456789-") || theForm.sacadoCep.value == ""))//|| (theForm.sacadoCep.value.length < 9))
    {
    ...
    /*
     Se a data for 00/00/0000 � como "v�lida" 
     */
    ...
    /*
     ...
     {
     alert("Informar o campo N�mero do Documento com 15 d�gitos num�ricos");
    ...
    for (var i = 1; i <= 4; i++) {
        ...
        {
            alert("Informar o campo \"Mensagem da ficha de compensa��o " + i + "\" com no maximo 60 caracteres.");
...
function MascaraCNPJ(keypress, valorCNPJ) {
    ...
    conjunto5 = 2;
    if ((caracteres.search(String.fromCharCode(keypress)) != -1)
            && (valorCNPJ.value.length < (conjunto1 + conjunto2 + conjunto3 + conjunto4 + conjunto5 + 1))) {
        if (valorCNPJ.value.length == conjunto1)
            valorCNPJ.value = valorCNPJ.value + separacao2;
        if (valorCNPJ.value.length == conjunto1 + conjunto2)
            valorCNPJ.value = valorCNPJ.value + separacao2;
        if (valorCNPJ.value.length == conjunto1 + conjunto2 + conjunto3)
            valorCNPJ.value = valorCNPJ.value + separacao3;
        if (valorCNPJ.value.length == conjunto1 + conjunto2 + conjunto3 + conjunto4)
            valorCNPJ.value = valorCNPJ.value + separacao1;
    }
    else {
        event.returnValue = false;
    }
